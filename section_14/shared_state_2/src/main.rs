...
// NOW WE CREATED A DEADLOCK !!!
	...
	for _ in 0..8 {
		...
		let handle = std::thread::spawn(move || {
			...
			// So now here we're saying "Hey, i want we're giving the lock to num"
			// But now we have num2 also requesting that lock !
			// We're going to get stuck here as nu2 is waiting for the lock
			// BUT num hols it, num cannot give it up because
			// it cannot finish running its execution.
			//
			// So we're just NEVER going to be able to finish running this !
			// It's a really simple example of whatr a deadlock is !!!!
			*num += 1;
		});
		handles.push(handle);
	}
